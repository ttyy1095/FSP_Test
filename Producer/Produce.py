...
class Produce(object):
    ...
    def ClientConnected(
        ...
        """
        listenthread = self._CreateListenThread(
            messageSequence, pb_sc, "ClientConnectedRsp", response_topic)
    ...
    def ClientDisconnected(
        ...
        '''
        listenthread = self._CreateListenThread(
            messageSequence, pb_sc, "ClientDisconnectedRsp", response_topic)

        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            cpbuf = pbbuf.ClientDisconnected(
    ...
    def PublishStream(
            ...
            response_topic):
        listenthread = self._CreateListenThread(
            messageSequence, pb_sc, "PublishStreamRsp", response_topic)
    ...
    def SetStreamSourceServer(
        ...
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.SetStreamSourceServer(
                messageSequence,
                stream_id,
                service_instance_id,
                uuid,
    ...
    def NotifyStreamPublished(
        ...
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.NotifyStreamPublished(
                messageSequence,
                client_id,
                stream_id,
                group_id,
    ...
    def GetSuperiorStreamServer(
        ...
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.GetSuperiorStreamServer(
                messageSequence, stream_id, service_instance_id, uuid, response_topic)
            self.producer.send(recv_topic, buf)
            rspdata = self._getRspdata(listenthread)
            return rspdata

    def GetStreamServersCP(
            self,
            messageSequence,
            topic_name,
            stream_id,
    ...
    def NotifyStreamSendingStart(
        ...
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.NotifyStreamSendingStart(
                messageSequence, recv_client_id, stream_id, uuid, response_topic)
        self.producer.send(recv_topic, buf)
    ...
    def QueryDB_clientDisconnected(
            self,
            client_id,
            service_instance_id,
            app_id):
        ...
        print query_result
        if query_result:
            print "find client id: %s in db successfully in clients:%s" % (
                client_id, app_id)
            checkresult2 = False
        else:
            print "fail to find client id: %s in db " % client_id
    ...
    def GetStream(
        ...
        """
        listenthread = self._CreateListenThread(
            messageSequence, pb_gc, "GetStreamRsp", response_topic)
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.GetStream(
    ...
    def JoinGroup(
            ...
            response_topic):
        listenthread = self._CreateListenThread(
            messageSequence, pb_gc, "JoinGroupRsp", response_topic)
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.JoinGroup_GC(
                messageSequence,
                group_id,
                user_id,
                group_token,
    ...
    def QuitGroup(
            ...
            response_topic):
        listenthread = self._CreateListenThread(
            messageSequence, pb_gc, "QuitGroupRsp", response_topic)
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.QuitGroup_GC(
                messageSequence,
                group_id,
                user_id,
                service_instance_id,
    ...
    def DestroyGroup(
            ...
            response_topic):
        listenthread = self._CreateListenThread(
            messageSequence, pb_gc, "DestroyGroupRsp", response_topic)
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.DestroyGroup_GC(
                messageSequence, group_id, response_topic)
            self.producer.send(recv_topic, buf)
            rspdata = self._getRspdata(listenthread)
            return rspdata

    def NotifyPublishStream(
            self,
            recv_topic,
            messageSequence,
            user_id,
    ...
    def __init__(
        ...
        if self.lock.acquire():
            self.consumer = KafkaConsumer(
                self.topic,
                bootstrap_servers=self.kafkaCluster,
                api_version=(
                    0,
                    10,
                    1))
