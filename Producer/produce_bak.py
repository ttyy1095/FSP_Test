...
class Produce(object):
    ...
    def CreateGroup(self, recv_topic, messageSequence, app_id, service_type, room_id,
                    company_id, response_topic):
        """
        由AS向GC调用
        ...
        if self.lock.acquire():
            ...
            gcbuf = PB_GC.CreateGroup(messageSequence, app_id, service_type, room_id,
                                      company_id, response_topic)
    ...
    def GetGroupServers(self, recv_topic, messageSequence, group_id, client_ip, user_id,
                        app_id, company_id, room_id, type, response_topic):
        """
        由AS向GC调用
        ...
        listenthread = self._CreateListenThread(messageSequence, pb_gc,
                                                "GetGroupServersRsp", response_topic)

        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            gcbuf = PB_GC.GetGroupServers(messageSequence, group_id, client_ip, user_id,
                                          app_id, company_id, room_id, type,
                                          response_topic)
            self.producer.send(recv_topic, gcbuf)
    ...
    def ClientConnected(self, recv_topic, messageSequence, client_id, service_instance_id,
                        app_id, client_name, uuid, response_topic):
        """
        由GS向SC调用
        ...
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            cpbuf = PB_SC.ClientConnected(messageSequence, client_id, service_instance_id,
                                          app_id, client_name, uuid, response_topic)
    ...
    def ClientDisconnected(self, recv_topic, messageSequence, client_id,
        ...
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            cpbuf = pbbuf.ClientDisconnected(messageSequence, client_id,
                                             service_instance_id, response_topic)
            self.producer.send(recv_topic, cpbuf)
            rspdata = self._getRspdata(listenthread)
            return rspdata

    def CreateStream(self, recv_topic, messageSequence, app_id, stream_type,
                     stream_property, uuid, response_topic):
        """
        ...
        if self.lock.acquire():
            ...
            createstream_buf = pbbuf.CreateStream(
                messageSequence, app_id, "Enum" + stream_type + "Stream",
                "Enum" + stream_property, uuid, response_topic)
    ...
    def SetStreamSourceServer(self, recv_topic, messageSequence, stream_id,
        ...
        if self.lock.acquire():
            sleep(1)
            self.lock.release()
            buf = pbbuf.SetStreamSourceServer(messageSequence, stream_id,
                                              service_instance_id, uuid, response_topic)
            self.producer.send(recv_topic, buf)
            rspdata = self._getRspdata(listenthread)
            return rspdata
...
if __name__ == '__main__':
    ...
    # resp = a.CreateGroup('gc_group_01', 1, 'app_hj', 0, '80664', '43150', 'lizzietest')
